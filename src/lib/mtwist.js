...
(function () {
	// from https://github.com/jawj/mtwist
	window.MTwist = class MTwist {
		...
		constructor(seed = Math.random() * 4294967295) {
			...
			for (mti = k = 1; k < 624; mti = ++k) {
				this.mt[mti] =
					(uint32mul(
						1812433253,
						this.mt[mti - 1] ^ (this.mt[mti - 1] >>> 30),
					) +
						mti) >>>
					0;
			}
		...
		randomUint32() {
			...
			if (this.mti >= 624) {
				...
				y =
					((this.mt[623] & 0x80000000) |
						(this.mt[0] & 0x7fffffff)) >>>
					0;
				this.mt[623] =
		...
		randomIntBelow(maxPlusOne) {
			// [0,n) with proper uniform distribution (see WARNING at http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/efaq.html)
		...
		randomIntBetween(inclusiveMin, inclusiveMax) {
			...
			return (
				inclusiveMin +
				this.randomIntBelow(inclusiveMax - inclusiveMin + 1)
			);
		}

		static test() {
			// returns true if answer is as calculated using official C implementation
			...
			for (i = k = 0; k < 1000; i = ++k) {
				...
				iterations = Math.floor(
					mtwist.randomUint32() / iterationFactor,
				);
				for (
					j = l = 0, ref = iterations;
					0 <= ref ? l < ref : l > ref;
					j = 0 <= ref ? ++l : --l
				) {
					mtwist.randomUint32();

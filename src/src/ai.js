...
(function () {
	...
	window.csonify = function (obj, indent = 0) {
	...
	ais.useAi = function (aiName, side = "beta") {
		if (!aiName) {
			return;
		}
	...
	ais.export = function (aiName) {
		var aiArray, key, l, n, name, ref, row, spec, tab;
		ref = commander.fleet.ais;
		for (key in ref) {
			name = ref[key];
			[row, tab] = getAIKey(key);
			if (name === aiName) {
				aiArray = [];
				for (n = l = 0; l < 10; n = ++l) {
					try {
						spec = JSON.parse(
							commander.fleet[getAIKey(row, n, tab)],
						);
						aiArray.push(spec);
					} catch (error) {
						aiArray.push("");

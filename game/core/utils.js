...
(function () {
	...
	window.track = function (name, kargs) {
		...
		if (
			window.location &&
			window.location.href.indexOf("gamedev.html") !== -1
		) {
			return;
		}
	...
	// fix stupid conflict with one cup
	...
	console.warnOnce = function (...args) {
		var message;
		// Displays a warning message only once
		message = JSON.stringify(args);
		if (!shownMessages.has(message)) {
			console.warn(...args);
			return shownMessages.add(message);
		}
	};

	stats.fpsAdd = function (n = 1) {
		return stats.add(stats.fps, n);
	};

	stats.netAdd = function (n = 1) {
		return stats.add(stats.net, n);
	};

	stats.simAdd = function (n = 1) {

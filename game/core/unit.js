		...
		class Unit {
			...
			draw() {
				...
				if (this.maxShield > 0) {
					...
					if (s > 0) {
						color = [
							...
							this.color[1],
							this.color[2],
							((255 * this.shield) / this.maxShield) * s,
						];
				...
				if (this.energy / this.storeEnergy < 0.05) {
					...
					baseAtlas.drawSprite(
						...
						[this.pos[0], this.pos[1] + 100 + this.radius],
						[1, 1],
						0,
						[255, 100, 100, t * 200],
					);
				...
				if (control.debug && this._pos2 != null && this._pos) {
					a = 16;
					_offset[0] =
						this._pos2[0] + (this._pos[0] - this._pos2[0]) * a;
					_offset[1] =
						this._pos2[1] + (this._pos[1] - this._pos2[1]) * a;
					baseAtlas.drawSprite(
						"img/pip1.png",
						this._pos,
						[1, 1],
			...
			thumb() {
				...
				if (!thumb_cache[k]) {
					...
					this.draw();
					if (
						((ref1 = this.aiRules) != null ? ref1.length : void 0) >
						0
					) {
						baseAtlas.drawSprite(
							"img/pip1.png",
			...
			drawSelection() {
				...
				// remove finished pre orders
				while (
					this.preOrders.length > 0 &&
					this.preOrders[0].id < this.onOrderId
				) {
					this.preOrders.shift();
				}
				// dont display orders if new ordres are in process of being given
				...
				if (this.owner === commander.number) {
					...
					for (i = j = 0, len = orders.length; j < len; i = ++j) {
						...
						if (order.type === "Move") {
							...
							angle = v2.angle(
								v2.sub(order.dest, prev, v2.create()),
							);
			...
			drawHPBar() {
				...
				number =
					Math.floor(this.maxHP / healthScale) +
					Math.floor(this.maxShield / healthScale);
				results = [];
				for (
					i = j = 0, ref = number;
					0 <= ref ? j < ref : j > ref;
					...
					if (i < Math.floor(this.maxHP / healthScale)) {
						...
						if (
							i <
							((this.cloak / this.mass) * this.maxHP) /
								healthScale
						) {
							color[3] = 100;
		...
		class Part {
			...
			draw() {
				...
				if (this.canShowDamage && this.image === this.orignalImage) {
					...
					showDamage =
						num / numParts > this.unit.hp / this.unit.maxHP;
					if (showDamage) {
						t = ((this.partNum + id) % 3) + 1;
						this.image = this.orignalImage.replace(
							".png",
							`.d${t}.png`,
						);
					}
		...
		class Engine extends Part {
			preDraw() {
				...
				if (v2.mag(this.unit.vel) > 1) {
					...
					// Only expand trail if units is moving more them 1 m/s

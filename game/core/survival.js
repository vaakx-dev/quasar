...
(function () {
	...
	survival.simulate = function (sim) {
		...
		if (sim.state === "running") {
			if (
				(sim.step / 16 - survival.firstWave) % survival.waveFreq ===
				0
			) {
				sim.waveNum += 1;
				aiMoney = Math.floor(
					survival.startAIMoney +
						Math.pow(survival.waveAIPow, sim.waveNum) *
							survival.waveAIMoneyMul,
				);
	...
	survival.genSurvival = function () {
		...
		results = [];
		for (
			i = j = 0, ref = survival.numComPoints;
			0 <= ref ? j < ref : j > ref;
			i = 0 <= ref ? ++j : --j
		) {
